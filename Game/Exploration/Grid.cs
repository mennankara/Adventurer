...
namespace Adventurer.Game.Exploration
{
    public abstract class Grid<T> : IGrid<T> where T : INode
    {
        ...
        private void CreateGrid()
        {
            ...
            //if (InnerGrid == null)
            //{
                Logger.Debug("[{0}] Creating grid [{1},{1}] ZetaWorldId={2} AdvDiaWorldId={3}", GetType().Name, GridBounds, worldId, AdvDia.CurrentWorldDynamicId);
                ...
                {
                    Logger.Debug("[{0}] Importing Current World Data from SceneStorage", GetType().Name);
        ...
        {
            Logger.DebugSetting($"Update called for {this.GetType().Name}");
