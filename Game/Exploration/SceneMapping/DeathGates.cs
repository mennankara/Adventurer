...
namespace Adventurer.Game.Exploration.SceneMapping
{
    public static class DeathGates
    {
        ...
        public static GateSide GetSide(DeathGateScene scene, Vector3 position)
        {
            ...
            var exitIsCloser = scene.ExitPosition.Distance(position) < scene.EnterPosition.Distance(position);
        ...
        public static Vector3 GetBestGatePosition(Vector3 destination)
        {
            ...
            if (!playerInEnterRegion && destinationInEnterRegion && !playerInExitRegion)
            {
                ...
                if (regionEnterScene != null)
                    return regionEnterScene.EnterPosition;
            }
            // go into exit region
        ...
        {
            Logger.DebugSetting($"TrySelectGate Side={side} Depth={targetDepth}");
            ...
            if (side == GateSide.DeepSide)
            {
                switch (targetDepth)
                {
                    case SceneDepth.Deeper:
                        bestGatePosition = NextGateScene.EnterPosition;
                        return true;
                    case SceneDepth.Same:
                        bestGatePosition = CurrentGateScene.ExitPosition;
                        return true;
            ...
            if (side == GateSide.ShallowSide)
            {
                ...
                {
                    case SceneDepth.Deeper:
                        bestGatePosition = CurrentGateScene.EnterPosition;
                        return true;
                    case SceneDepth.Same:
                        bestGatePosition = CurrentGateScene.EnterPosition;
                        return true;
                    case SceneDepth.Shallower:
                        bestGatePosition = PreviousGateScene.ExitPosition;
                        return true;
                    case SceneDepth.NotFound:
                        bestGatePosition = CurrentGateScene.EnterPosition;
                        return true;
