...
namespace Adventurer.Coroutines.RiftCoroutines
{
    public class RiftCoroutine : IDisposable, ICoroutine
    {
        ...
        public virtual async Task<bool> GetCoroutine()
        {
            ...
            {
                Logger.Debug("[Rift] Someone told us to stop rifting, so we will do what we're told like a good boy and/or girl.");
        ...
        private async Task<bool> OpeningRift()
        {
            ...
            var settings = Settings.PluginSettings.Current;
            ...
            if (maximizeXp)
            {
                Logger.Info("Opening Normal Rift for XP Shrine", _RiftType);
            ...
            else
            {
                ...
                if (_RiftType == RiftType.Greater && shouldEmpower && canEmpower && Settings.PluginSettings.Current.UseEmpoweredRifts)
                {
        ...
        private async Task<bool> EnteringRift()
        {
            ...
            else
            {
                ...
                Logger.Debug("Expecting to find a portal here but didnt find one. InTown={0}", inTown);
                if(inTown)
                    State = States.InTown;
                else
                    State = States.Failed;
        ...
        private bool EnteringGreaterRift()
        {
            if (ZetaDia.IsLoadingWorld || AdvDia.CurrentWorldId == ExplorationData.ActHubWorldIds[Act.A1])
            {
                ...
                {
                    Logger.Debug("Stuck detected entering portal, maybe interaction coroutine has failed to do its job");
